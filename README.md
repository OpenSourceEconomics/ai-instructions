...
Claude Code; ignored as plain text by Codex, Copilot, Cursor), then project-specific
content below:
...
`AGENTS.md` is the only instruction file; don't add `CLAUDE.md` or `GEMINI.md` wrappers.
Claude Code reads `AGENTS.md` only when no `CLAUDE.md` exists in the project or any
directory above it, so remove leftover wrappers (`/boilerplate-update` flags them).
