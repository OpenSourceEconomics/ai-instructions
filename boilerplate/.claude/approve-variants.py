...
Example: "timeout 60 PYTHONPATH=. pytest" is approved as:
    wrapper(timeout) + wrapper(env vars) + safe_command(pytest)
...
Registered in .claude/settings.local.json:
    ...
    "hooks": {
      "PreToolUse": [{
        "matcher": "Bash",
        "hooks": [{"type": "command", "command": "python3 .claude/approve-variants.py"}]
      }]
    }
...
DEBUG
-----
    echo '{"tool_name": "Bash", "tool_input": {"command": "timeout 30 pytest"}}' \
        | python3 .claude/approve-variants.py
"""
...
SAFE_COMMANDS = [
    ...
    (r"^uvx\b", "uvx"),
    # pixi
    (r"^pixi\s+(run|install|add|remove|update|lock|info|list|shell|init)\b", "pixi"),
    # prek (pre-commit wrapper)
    (r"^prek\s+(run|install|uninstall|autoupdate)\b", "prek"),
    # make
    (r"^make\b", "make"),
