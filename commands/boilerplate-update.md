...
7. **Compare and report deviations.** For each file, compare against the tier-appropriate
   ...
   - **Notebook/path/separator hooks**: If the project has `.ipynb` files, ensure the
     ...
     heavily on older code — if a project has many pre-existing violations, propose an
     `exclude:` for the legacy paths rather than omitting the hook.
   - **codespell**: ensure the hook and a `[tool.codespell]` section exist. Standard
     econ/numerics jargon is not flagged, so an over-long `ignore-words-list` is a smell;
     German-language projects legitimately need more entries.
   ...
   - **GitHub Actions versions**: For each `.github/workflows/*.yml`, check ALL versioned
     ...
     Resolve the latest version from the releases API, not from this repo's templates:

     ```bash
     gh api repos/prefix-dev/pixi/releases --jq '.[0].tag_name'
     gh api repos/prefix-dev/setup-pixi/releases --jq '.[0].tag_name'
     gh api repos/actions/checkout/releases --jq '.[0].tag_name'
     ```

     **Never take a CI version from `boilerplate/README.md`.** Its workflow blocks are
     illustrative and are refreshed by different commits than the pre-commit hook versions,
     so the submodule can sit at `origin/main` while its CI block is weeks old. A project
     is frequently *ahead* of the template here, and copying the template's number would
     **downgrade** it. Compare project ↔ resolved-latest, never project ↔ template.
   - **`pixi-version:` must match the local pixi that writes `pixi.lock`.** This is a
     correctness check, not a currency check: the lock is generated by whatever pixi the
     developer runs, and CI then validates it with the pinned binary. If local is *newer*
     than the pin, CI is relying on an older pixi reading a newer lock — which often works
     (pixi is forward-tolerant), so it fails quietly or not at all until it doesn't. Run
     `pixi --version`, compare, and flag any gap. The reverse direction (CI newer than
     local) is safe, since pixi reads older lock formats. Pinning both to latest satisfies
     this and currency at once.
   - **Report back on the templates themselves.** This command only ever flags the
     *project*, so a stale template silently seeds every newly bootstrapped project at the
     same stale versions and nothing in the loop catches it. Once you have resolved the
     latest versions above, compare them against the workflow blocks in
     `boilerplate/README.md` too, and if those are behind, say so explicitly in the report
     as a fix needed in `ai-instructions` (not in the project). Same for hook versions in
     the template's `.pre-commit-config.yaml` examples.
